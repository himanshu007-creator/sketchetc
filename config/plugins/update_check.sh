...
if [ "$SENDER" = "mouse.clicked" ]; then
  ...
  row skip 󰅖 "Skip this version" "$CONFIG_DIR/plugins/state_cli.sh set update_skip '$REMOTE'; sketchybar --set updater drawing=off"
  toggle_popup
  exit 0
fi
...
if [ "${BEHIND:-0}" -gt 0 ] && [ "$(cat "$(state_get update_skip)" 2>/dev/null)" != "$REMOTE" ]; then
  ...
  if [ ! -f "$(uc_runtime ".update_notified_$REMOTE")" ]; then
    for old in "$USER_CONF_DIR"/state/.update_notified_*; do
      [ -e "$old" ] && [ "$old" != "$(uc_runtime ".update_notified_$REMOTE")" ] && rm -f "$old"
    done
    touch "$(uc_runtime ".update_notified_$REMOTE")"
